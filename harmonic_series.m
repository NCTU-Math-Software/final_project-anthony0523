...
%輸入一個x屬於(1,33.8)，此方程式將輸出一個N屬於正整數並滿足S(N+1)>x>sN (如果x>33.8，1/N會開始小於machine epsilon，程式會判斷為0，無法找到N)
function harmonic_series(x)                     
    ...
    elseif x > 27.5 && x < 33.8                 %如果x>27.5，計算時間將超出10分鐘
    ...
    elseif x >= 33.8
        disp('x >= 33.8，無法顯示答案，因為1/N已開始小於machine epsilon，將被判斷為0，即無法找到N');
        x = input('請重新輸入x，範圍為(1,33.8):');
    end
