%輸入一個x屬於(1,710)，此方程式將輸出一個N屬於正整數並滿足S(N+1)>x>sN (如果x>33.8，1/N會開始小於machine epsilon，程式會判斷為0，故開始用Euler-Maclaurin sum找估計值)
function harmonic_series(x)                     
    ...
    while sum_all < x                           %設定條件為總和小於x時繼續做
        if x>= 33.8                             %x >= 33.8，因為1/N已開始小於machine epsilon，將被判斷為0
            N = exp(x - gamma);                 %用Euler-Maclaurin sum來估計N
            break
        end
    ...
    elseif x >= 33.8                            %如果x>=33.8，輸出估計值
        ...
        disp('用Euler-Maclaurin sum, 我們可以得到估計值:');
        disp('N=');
        disp(N);
    else                                        %正常情況之輸出
        disp(['S' num2str(N-2) ' =']);          %輸出SN
